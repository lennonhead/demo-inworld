...
namespace Inworld.Sample
{
    public class DemoCanvas : MonoBehaviour
    {
        ...
        void Update()
        {
            if (!InworldController.Player)
                return;
            InworldPlayer player = InworldController.Player.GetComponent<InworldPlayer>();
            if(!string.IsNullOrEmpty(player.WeatherCity)){
                m_Content.text = $"Current Weather In {player.WeatherCity}\n{player.WeatherStatus}";
            }
        }
    }
}
